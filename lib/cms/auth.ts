...
export function getAuthUser(request: Request): { email: string; role: UserRole } | null {
  ...
  const user = getUserByEmail(verified.email);
  if (!user || user.role !== verified.role) return null;
  return { email: verified.email, role: verified.role };
}

export function requireAuth(request: Request): { email: string; role: UserRole } | Response {
...
): { email: string; role: UserRole } | Response {
  const user = getAuthUser(request);
  if (!user) {
    return new Response(JSON.stringify({ error: "Unauthorized" }), {
      status: 401,
