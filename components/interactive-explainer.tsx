...
const PRODUCTS: Record<string, { title: string; sub: string }> = {
  os: {
    title: "MentivisOS",
    sub: "Le programme exact qui comble l'écart, généré en moins d'une minute.",
  },
  talent: {
    title: "TalentOS",
    sub: "Recrutement calibré par compétences. Pas par intuition.",
  },
  api: {
    title: "Mentivis API",
    sub: "Connecté à vos outils. Sans refonte d'organisation.",
  },
};
...
export default function InteractiveExplainer({ lang }: InteractiveExplainerProps) {
  const t = getT(lang);
    ...
    <section ref={sectionRef} className="section" style={{ background: "var(--bg-warm)" }}>
      ...
      <div className="container">
        ...
        >
          {/* TOP BAR */}
          <div
            className="explainer-top-bar-desktop"
            style={{
              display: "flex",
          ...
          <div className="explainer-mobile-product-selector" style={{ display: "none", marginBottom: 20 }}>
            ...
            >
              <option value="os">MentivisOS</option>
              <option value="talent">TalentOS</option>
              <option value="api">Mentivis API</option>
            </select>
          </div>
            ...
            >
              {ORBS.map((orb, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="explainer-orb-container"
                    data-pos={pos}
                    onClick={(e) => {
                      const target = e.target as HTMLElement;
                      if (target.closest(".explainer-tap-zone")) {
                            ...
                            {Array.from({ length: 50 }).map((_, wi) => (
                              <span
                                ...
                                style={{
                                  ...
                                  animation: `explainerWaveform ${0.25 + Math.random() * 0.35}s ease-in-out infinite alternate`,
          ...
          <Link
            href={`/${lang}/demo`}
            className="explainer-mobile-cta"
            style={{
              display: "none",
              background: "#0A0A0A",
              color: "#FFFFFF",
      ...
      <style>{`
        ...
        @keyframes explainerWaveform {
          0% { transform: scaleY(0.3); }
          100% { transform: scaleY(1); }
        }
        @media (max-width: 900px) {
          .explainer-stage { flex-direction: column; gap: 24px; }
          .explainer-title { text-align: left; }
        }
        @media (max-width: 768px) {
          .explainer-nav { display: none !important; }
          .explainer-arrows { display: none !important; }
          .explainer-orb-container[data-pos="1"] {
          ...
          .explainer-orb-container[data-pos="2"],
          .explainer-orb-container[data-pos="-2"],
          .explainer-orb-container[data-pos="3"],
          .explainer-orb-container[data-pos="-3"] {
            opacity: 0 !important;
            pointer-events: none !important;
          }
          .explainer-features-nav {
            overflow-x: auto;
          ...
          .explainer-features-nav::-webkit-scrollbar { display: none; }
          .explainer-cta-link {
            display: none !important;
          }
          .explainer-mobile-cta {
            display: flex !important;
            width: 100%;
          }
          .explainer-mobile-product-selector {
            display: block !important;
          }
          .explainer-top-bar-desktop {
            display: none !important;
          }
        }
      `}</style>
